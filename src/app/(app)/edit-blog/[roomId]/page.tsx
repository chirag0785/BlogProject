      ...
      return (
        <div className="flex items-center justify-center h-screen">
          <div className="text-center">
            <h1 className="text-4xl font-semibold text-gray-800 dark:text-gray-200">{error}</h1>
          </div>
        </div>
      )
  }
      ...
      <>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 max-w-xl mx-auto p-4">
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-gray-800 dark:text-gray-200">Topic</FormLabel>
                  <Select onValueChange={(ev)=> field.onChange(ev)} defaultValue={field.value}>
                    <FormControl>
                      <SelectTrigger className="w-full border-gray-300 rounded-md dark:border-gray-600 dark:bg-gray-700 dark:text-white">
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  <FormLabel className="text-gray-800 dark:text-gray-200">Heading</FormLabel>
                  <FormControl>
                    <Input
                      placeholder="Enter the heading"
                      className="border-gray-300 rounded-md shadow-sm dark:border-gray-600 dark:bg-gray-700 dark:text-white"
                      {...field}
                  ...
                  <div className="mt-4 space-y-2">
                    {suggestedHeadings.map((heading, idx) => (
                      <div
                        ...
                        key={idx}
                        className="text-sm text-gray-800 dark:text-gray-100 bg-gray-50 dark:bg-gray-800 border border-gray-200 dark:border-gray-600 p-3 rounded-lg shadow-sm hover:bg-gray-100 dark:hover:bg-gray-700 transition duration-150 ease-in-out cursor-pointer"
                      >
                        {heading}
                      </div>
                    ))}
        ...
        <Room roomId={params.roomId} initialContent={blog?.content}>
          <div className="space-y-6 p-6 bg-gray-50 dark:bg-gray-800 min-h-screen w-full">
            ...
            {isOpen && (
              <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
                <div className="bg-white p-8 max-w-lg w-full mx-auto rounded-lg shadow-2xl space-y-6 dark:bg-gray-800">
                  <div className="flex justify-between items-center">
                    <h3 className="text-xl font-bold text-gray-800 dark:text-gray-100">Share Options</h3>
                    <Button
                      onClick={closeModal}
                      variant="ghost"
                  ...
                  <div className="flex justify-end gap-4">
                    <Button
                      variant="outline"
                      onClick={closeModal}
                      className="px-4 py-2 text-sm text-gray-700 border-gray-300 rounded-md hover:bg-gray-100 dark:text-gray-300 dark:border-gray-600 dark:hover:bg-gray-700"
                    >
                      Cancel
                    </Button>
                  </div>
            ...
            <TextEditor initialContent={blog?.content} roomId={params.roomId}/>
          </div>
        </Room>
      </>
  )  
