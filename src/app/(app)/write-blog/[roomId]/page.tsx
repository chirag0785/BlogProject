...
const Page = ({ params }: { params: { roomId: string } }) => {
  ...
  return (
    <>
      <Button className='absolute top-4 right-4 dark:text-gray-300 text-white bg-red-500'
        onClick={()=> {
          router.push('/dashboard');
        }}>Leave Room</Button>
      <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6 max-w-xl mx-auto p-4 dark:bg-gray-900 dark:text-white">
          <FormField
            control={form.control}
            name="topic"
            render={({ field }) => (
              <FormItem>
                <FormLabel className="dark:text-gray-300">Topic</FormLabel>
                <Select onValueChange={field.onChange} defaultValue={field.value}>
                  <FormControl>
                    <SelectTrigger className="w-full border-gray-300 rounded-md dark:border-gray-600 dark:bg-gray-800">
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel className="dark:text-gray-300">Heading</FormLabel>
                <FormControl>
                  <Input
                    placeholder="Enter the heading"
                    className="border-gray-300 rounded-md shadow-sm dark:border-gray-600 dark:bg-gray-800 dark:text-white"
                    {...field}
      ...
      <Room roomId={params.roomId} initialContent=''>
        <div className="space-y-6 p-6 bg-gray-50 min-h-screen w-full dark:bg-gray-900 dark:text-white">
          ...
          {isOpen && (
            <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
              <div className="bg-white p-8 max-w-lg w-full mx-auto rounded-lg shadow-2xl space-y-6 dark:bg-gray-800">
                <div className="flex justify-between items-center">
                  <h3 className="text-xl font-bold text-gray-800 dark:text-white">Share Options</h3>
                  <Button
                    onClick={closeModal}
                    variant="ghost"
          ...
          <TextEditor roomId={params.roomId} initialContent='' />
        </div>
      </Room>
    </>
  )
